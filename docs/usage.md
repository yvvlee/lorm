...
- [6. Core CRUD Operations](#6-core-crud-operations)
  ...
  - [Update](#update)
  - [Delete](#delete)
  - [Raw SQL Reads and Writes](#raw-sql-reads-and-writes)
...
| `version` | Enables optimistic locking. Automatically incremented on `UPDATE`. Max 1 per model. | Built-in integers of at least 32 bits: `int32`, `uint32`, `int64`, `uint64`, `int`, `uint`, their type aliases, and single pointers |
| `json` | Serializes/deserializes the field as JSON (using fast JSON codecs). | Structs, slices, maps |

`created`, `updated`, and `version` are mutually exclusive. A primary key cannot also be an `updated` or `version` field. Auto-increment fields cannot use managed time or version tags. `json` cannot be combined with auto-increment, managed time, or version tags. Conflicting tags and grouped declarations carrying a `lorm` tag fail during generation.

Embedded structs are flattened recursively across files and packages. Nil embedded pointers are initialized as needed when reading field values, inserting, updating, or scanning. Their children use their own values or zero values, rather than becoming SQL `NULL` because an ancestor was nil. Ordinary pointer fields retain their field-specific null behavior, including those inside embedded structs. Nested database prefixes are concatenated from outermost to innermost. Recursive embedding fails during generation.

### Struct Embedding & Flattening Rules

- **Private Fields**: The generator skips all unexported fields, including unexported embedded fields and their children. Their types and tags are not validated.
- **Model Types**: Models declaring type parameters are rejected during generation. Ordinary generic helper types remain supported.
- **Byte Fields**: Model fields cannot use `sql.RawBytes`, its aliases, or pointers to it, including fields tagged `json`. Generation reports an error and recommends `[]byte`.
- **Name Conflicts**: Flattened fields must produce unique column accessor methods and cannot use the accessor names `All` or `WithAlias`. Direct model fields, including embedded fields, cannot share names with methods generated for that model. Generation reports the model, fields, and conflicting name.
...
When selecting individual files, the generator also loads sibling Go source files to resolve embedded types and type aliases. It uses the same source selection rules as directory generation, excluding test files and generated files. Only selected files receive generated output; `--ignore` filters generation targets, not source dependencies.

All input directories are loaded, validated, and formatted in memory before any output file is written. Failures in these stages do not create or overwrite output files. Filesystem errors during writing are returned immediately.
...
// Standard batch insert, at most 1000 models per statement by default:
...
`BatchSize(n)` sets the maximum number of models per INSERT and requires a positive size. The default is `lorm.DefaultInsertBatchSize` (1000). For example, use `engine.Insert[*User]().BatchSize(500).AddModels(users...).Exec(ctx)`. Split batches share a transaction. An existing transaction is reused; propagate errors to its owner to trigger rollback. Failed execution returns zero rows and an error. Reduce the batch size for wide models to stay within the database parameter limit. `Exec` resets the size to the default; `Clone` retains it.

Ordinary multi-model inserts never backfill IDs, even when the final batch contains one model. `RequireIDBackfill()` always executes one model per statement. If a generated ID is needed but the dialect supports neither `RETURNING` nor `LastInsertId`, both single-model and multi-model inserts fail before executing SQL. Explicit IDs do not require backfill support. Rolling back a transaction does not undo IDs already written to models.
...
### Raw SQL Reads and Writes

Use `Engine.SQL` to read rows and `Engine.Exec` to execute writes when a query cannot be expressed with a model statement builder. Both methods reuse the transaction in `ctx` when one is active.

#### Reading Data (`Engine.SQL`)

`Engine.SQL` returns a standard-library `*sql.Rows`. Always close the result set and check the iteration error:
...
For models with field mappings generated by `lormgen`, use `ScanModel` and `ScanModels` to scan by column name. Use `ScanCol` and `ScanCols` for scalar results; both methods require the SQL result to contain exactly one column.

`GetCol`, `FindCols`, `PageCols`, and `ScanCols` reject `sql.RawBytes`, `*sql.RawBytes`, and aliases of these types using type assertions. Use `[]byte` to retain byte data; it is a distinct type and is accepted. `ScanCol` reads one row and leaves the result set open. Its `sql.RawBytes` values remain valid only until the next `Next`, `Scan`, or `Close` call.

```go
// ScanModel: read the first row into a model.
rows, err := engine.SQL(ctx,
...
```go
// ScanModels: read all rows into a model slice.
rows, err := engine.SQL(ctx,
...
```go
// ScanCol: read the single value from the first row.
rows, err := engine.SQL(ctx,
...
```go
// ScanCols: read the single value from every row.
rows, err := engine.SQL(ctx,
...
#### Writing Data (`Engine.Exec`)

`Engine.Exec` returns a standard-library `sql.Result`, from which you can retrieve the affected row count:
...
> **Note**: Raw SQL is passed to the database driver unchanged. Bind parameters instead of concatenating user input, and use placeholders for the selected driver: MySQL and SQLite use `?`, while PostgreSQL uses `$1`, `$2`. Raw writes are outside the full-table write protection provided by `Update.Exec` and `Delete.Exec`.
...
`try.TimeRange` binds `time.Time` values directly, preserving nanoseconds and time zones. CASE conditions and values must produce non-empty expressions; for example, `Case().When(builder.Or{}, "1")` returns an error. `UPDATE SET` values and expressions inside `INSERT VALUES` must also produce non-empty SQL. Empty `Or` groups, `Expr("")`, and `Expr("  ")` return an error. Plain strings and `nil` values are still bound as parameters.
...
Statement `Where`, `Having`, and `SetMap` reject map keys that become the same identifier after quoting, before executing SQL. The check also applies to `Eq` and `NotEq` inside nested `And` and `Or` groups.
...
Use `[]byte` for byte data returned by these methods because the result set is closed before they return.
...
The outermost `TX` or `TXWithOptions` call that starts a transaction cleans it up on exit, including callback errors, panics, and `runtime.Goexit()`. Panics continue to propagate, and `Goexit()` still terminates the current goroutine. Cleanup does not wait for `engine.Close()`. A completed transaction does not issue another rollback operation to the database.
...
Add a `version` tag to a built-in integer field of at least 32 bits. Generation rejects 8-bit and 16-bit integers:
