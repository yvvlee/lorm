...
Ordinary multi-model inserts never backfill IDs, even when the final batch contains one model. `RequireIDBackfill()` always executes one model per statement. Rolling back a transaction does not undo IDs already written to models.
...
Use `[]byte` to retain byte data from `ScanCols`. `ScanCol` reads one row and leaves the result set open. Its `sql.RawBytes` values remain valid only until the next `Next`, `Scan`, or `Close` call.
