...
func TestCaseWithComplexVal(t *testing.T) {
	...
	qb := Select().
		AddColumn(Alias(caseStmt, "complexCase")).
		From("table")
	sql, args, err := qb.ToSql()
...
func TestCaseWithNoVal(t *testing.T) {
	...
	qb := Select().AddColumn(caseStmt).From("table")
	sql, args, err := qb.ToSql()
...
func TestMultipleCase(t *testing.T) {
	...
	qb := Select().
		AddColumn(Alias(caseStmtNoval, "case_noval")).
		AddColumn(Alias(caseStmtExpr, "case_expr")).
		From("table")
